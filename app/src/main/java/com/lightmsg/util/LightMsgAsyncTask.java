...
/**
 * {@link AsyncTask} substitution for the aspark app. Modeled after
 * {@link AsyncTask}; the basic usage is the same, with extra features: - Bulk
 * cancellation of multiple tasks. This is mainly used by UI to cancel pending
 * tasks in onDestroy() or similar places. - Instead of
 ...
 * be called and whel it won't. Note this class is missing some of the
 * {@link AsyncTask} features, e.g. it lacks {@link AsyncTask#onProgressUpdate}.
 * Add these when necessary.
 */
public abstract class LightMsgAsyncTask<Params, Progress, Result> {
    ...
    /**
     * Tracks {@link LightMsgAsyncTask}. Call {@link #cancellAllInterrupt()} to
     * cancel all tasks registered.
     */
    public static class Tracker {
        private final LinkedList<LightMsgAsyncTask<?, ?, ?>> mTasks = new LinkedList<LightMsgAsyncTask<?, ?, ?>>();

        private void add(LightMsgAsyncTask<?, ?, ?> task) {
            synchronized (mTasks) {
                mTasks.add(task);
            }
        }

        private void remove(LightMsgAsyncTask<?, ?, ?> task) {
            synchronized (mTasks) {
                mTasks.remove(task);
            }
        }
        ...
        public void cancellAllInterrupt() {
            synchronized (mTasks) {
                for (LightMsgAsyncTask<?, ?, ?> task : mTasks) {
                    task.cancel(true);
                }
                mTasks.clear();
            }
         ...
         * {@code current} itself.
         */
        /* package */void cancelOthers(LightMsgAsyncTask<?, ?, ?> current) {
            final Class<?> clazz = current.getClass();
            synchronized (mTasks) {
                final ArrayList<LightMsgAsyncTask<?, ?, ?>> toRemove = new ArrayList<LightMsgAsyncTask<?, ?, ?>>();
                for (LightMsgAsyncTask<?, ?, ?> task : mTasks) {
                ...
                for (LightMsgAsyncTask<?, ?, ?> task : toRemove) {
                    mTasks.remove(task);
                }
            }
        }
        ...
        /* package */boolean containsTaskForTest(LightMsgAsyncTask<?, ?, ?> task) {
    ...
    private static class InnerTask<Params2, Progress2, Result2> extends
        ...
        private final LightMsgAsyncTask<Params2, Progress2, Result2> mOwner;

        public InnerTask(LightMsgAsyncTask<Params2, Progress2, Result2> owner) {
            mOwner = owner;
        }
    ...
    public LightMsgAsyncTask(Tracker tracker) {
        mTracker = tracker;
        if (mTracker != null) {
            mTracker.add(this);
        }
     ...
     */
    public final LightMsgAsyncTask<Params, Progress, Result> executeParallel(Params... params) {
     ...
     * @see AsyncTask#execute
     */
    public final LightMsgAsyncTask<Params, Progress, Result> executeSerial(Params... params) {
     ...
     */
    public final LightMsgAsyncTask<Params, Progress, Result> cancelPreviousAndExecuteParallel(
            Params... params) {
     ...
     */
    public final LightMsgAsyncTask<Params, Progress, Result> cancelPreviousAndExecuteSerial(
            Params... params) {
        return executeInternal(SERIAL_EXECUTOR, true, params);
    }

    private final LightMsgAsyncTask<Params, Progress, Result> executeInternal(Executor executor,
     ...
     */
    public static LightMsgAsyncTask<Void, Void, Void> runAsyncParallel(Runnable runnable) {
     ...
     */
    public static LightMsgAsyncTask<Void, Void, Void> runAsyncSerial(Runnable runnable) {
        return runAsyncInternal(SERIAL_EXECUTOR, runnable);
    }

    private static LightMsgAsyncTask<Void, Void, Void> runAsyncInternal(Executor executor,
            final Runnable runnable) {
        LightMsgAsyncTask<Void, Void, Void> task = new LightMsgAsyncTask<Void, Void, Void>(null) {
            @Override

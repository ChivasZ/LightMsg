...
{
    private static final String TAG = "SmartCommunity/" + UriMatcher.class.getSimpleName();
    ...
    {
        Log.d(TAG, "addURI()...authority="+authority+", path="+path+", code="+code);
        if (code < 0) {
        ...
        for (int i = -1; i < numTokens; i++) {
            ...
            if (j == numChildren) {
                ...
                if (token.equals("#")) {
                    child.mWhich = NUMBER;
                    Log.d(TAG, "addURI()...NUMBER");
                } else if (token.equals("*")) {
                    child.mWhich = TEXT;
                    Log.d(TAG, "addURI()...TEXT");
                } else {
                    child.mWhich = EXACT;
                    Log.d(TAG, "addURI()...EXACT");
                }
                child.mText = token;
          ...
          which_switch:
                switch (n.mWhich) {
                    case EXACT:
                        ...
                        Log.d(TAG, "match()...EXACT,"+j+", n="+n+", u="+u);
                        break;
                    case NUMBER:
                        int lk = u.length();
                        for (int k=0; k<lk; k++) {
                        ...
                        node = n;
                        Log.d(TAG, "match()...NUMBER,"+j+", n="+n+", u="+u);
                        break;
                    case TEXT:
                        node = n;
                        Log.d(TAG, "match()...TEXT,"+j+", n="+n+", u="+u);
                        break;
                }
                if (node != null) {
                    break;

...
class PacketReader {
    ...
    private void parsePackets(Thread thread) {
    	//Log.d("PacketReader", "parsePackets(), thread="+thread.getName());
        try {
            int eventType = parser.getEventType();
            do {
            	//Log.d("PacketReader", "parsePackets(), eventType="+eventType);
                if (eventType == XmlPullParser.START_TAG) {
                    int parserDepth = parser.getDepth();
                    ParsingExceptionCallback callback = connection.getParsingExceptionCallback();
                    
                    //Log.d("PacketReader", "parsePackets(), parser.name="+parser.getName());
                    ...
                    // the connectionID lock so that the packet reader startup can finish.
                        ...
                        if ("jabber:client".equals(parser.getNamespace(null))) {
                        	...
                        	//Log.d("PacketReader", "parsePackets(), parser.getAttributeCount()="+parser.getAttributeCount());
                            for (int i=0; i<parser.getAttributeCount(); i++) {
                            	//Log.d("PacketReader", "parsePackets(), parser.attributeName["+i+"]="+parser.getAttributeName(i));
                                if (parser.getAttributeName(i).equals("id")) {
                                    // Save the connectionID
                                    connectionID = parser.getAttributeValue(i);
                                    //Log.d("PacketReader", "parsePackets(), connectionID="+connectionID);

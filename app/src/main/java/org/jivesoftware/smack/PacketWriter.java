...
class PacketWriter {
    ...
    */ 
    ...
    private Packet nextPacket() {
    	...
    	//Log.d("PacketWriter", "nextPacket(), packet="+packet);
        return packet;
    }

    private void writePackets(Thread thisThread) {
    	//Log.d("PacketWriter", "writePackets(), thread="+thisThread.getName());
        try {
            // Open the stream.
            openStream();
            // Write out packets from the queue.
        	//Log.d("PacketWriter", "writePackets(), writerThread="+writerThread+", done="+done);
            while (!done && (writerThread == thisThread)) {
                Packet packet = nextPacket();
                if (packet != null) {
                	//Log.d("PacketWriter", "writePackets(), packet1="+packet.toXML());
            ...
            try {
            	//Log.d("PacketWriter", "writePackets(), !queue.isEmpty()="+!queue.isEmpty());
                while (!queue.isEmpty()) {
                    Packet packet = queue.remove();
                    //Log.d("PacketWriter", "writePackets(), packet2="+packet.toXML());
                    writer.write(packet.toXML());
                }
                writer.flush();
            }
            ...
            // Close the stream.
            try {
            	//Log.d("PacketWriter", "writePackets(), packet3=</stream:stream>");

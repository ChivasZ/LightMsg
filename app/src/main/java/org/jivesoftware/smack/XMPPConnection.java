...
public class XMPPConnection extends Connection {
	...
	protected void shutdown(Presence unavailablePresence) {
		Log.v("XMPPConnection("+this+")", "shutdown(), "+unavailablePresence);
		...
		connected = false;
		Log.v("XMPPConnection("+this+")", "shutdown(), set connected "+connected);
	...
	public synchronized void disconnect(Presence unavailablePresence) {
		Log.e("XMPPConnection("+this+")", "disconnect(), "+unavailablePresence);
		try {
			throw new Exception();
		} catch (Exception e){
			e.printStackTrace();
	...
	public void sendPacket(Packet packet) {
        Log.v("XMPPConnection("+this+")", "sendPacket(), "+packet);
		if (!isConnected()) {
			throw new IllegalStateException("Not connected to server.");
		}
		if (packet == null) {
	...
	private void initConnection() throws XMPPException {
		...
		Log.d("XMPPConnection("+this+")", "initConnection(), isFirstInitialization="+isFirstInitialization);
		...
		try {
			...
			connected = true;
			Log.v("XMPPConnection("+this+")", "initConnection(), 1, set connected "+connected);
			...
			authenticated = false;
			connected = false;
			Log.v("XMPPConnection("+this+")", "initConnection(), 2, set connected "+connected+", "+ex);
	...
	synchronized void notifyConnectionError(Exception e) {
		...
		if (packetReader == null || packetWriter == null) {
			Log.e("XMPPConnection("+this+")", "packetReader == null || packetWriter == null !!!");
			return;
		}

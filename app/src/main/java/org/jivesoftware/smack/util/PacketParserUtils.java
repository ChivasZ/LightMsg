...
public class PacketParserUtils {
    ...
    public static Packet parseMessage(XmlPullParser parser) throws Exception {
        ...
        if (message.getType() == Message.Type.file) {
            message.setFileMethod(Message.FileMethod.valueOf(parser.getAttributeValue("", "file-method")));
        }
        ...
        while (!done) {
            ...
            if (eventType == XmlPullParser.START_TAG) {
                        ...
                        namespace.equals(PROPERTIES_NAMESPACE))
                {
                    ...
                    String type = parser.getAttributeValue("", "content-type");
                    String hash = parser.getAttributeValue("", "hash");
                    Message.File mf = new Message.File(name, size, hash, type);
